...
for i in range(len(X)):
    ...
    def apso_objective(params):
        ...
        for param_set in params:
            ...
            lambda1 = max(0.01, min(0.98, param_set[2]))  # RBF权重
            lambda2 = max(0.01, min(0.98 - lambda1, param_set[3]))  # LINEAR权重
    ...
    # APSO优化设置
    # 参数范围：[C, gamma, lambda1(RBF), lambda2(LINEAR), degree, coef0, epsilon]
    bounds = (
        [0.01, 0.01, 0.01, 0.01, 2, 0, 0.01],  # 下界
        [100.0, 10.0, 0.98, 0.98, 4, 5, 1.0]  # 上界
    )

    # 初始化APSO优化器
    optimizer = APSO(n_particles=50, dimensions=7, bounds=bounds, max_iter=70)

    # 运行APSO优化
    best_cost, best_pos = optimizer.optimize(apso_objective)

    # 提取最佳参数
    best_C_apso, best_gamma_apso, best_lambda1_apso, best_lambda2_apso, best_degree_apso, best_coef0_apso, best_epsilon_apso = best_pos
    best_degree_apso = int(round(best_degree_apso))

    # 确保lambda1 + lambda2 <= 0.98
    ...
    best_lambda3_apso = 1 - best_lambda1_apso - best_lambda2_apso  # POLY的权重

    print(f"APSO最佳参数:")
    print(f"C: {best_C_apso:.4f}")
    print(f"gamma: {best_gamma_apso:.4f}")
    print(f"lambda1 (RBF权重): {best_lambda1_apso:.4f}")
    print(f"lambda2 (LINEAR权重): {best_lambda2_apso:.4f}")
    print(f"lambda3 (POLY权重): {best_lambda3_apso:.4f}")
    ...
    # C参数范围
    C_lower = max(0.01, best_C_apso * 0.8)
    C_upper = min(100.0, best_C_apso * 1.2)

    # gamma参数范围
    gamma_lower = max(0.01, best_gamma_apso * 0.8)
    gamma_upper = min(10.0, best_gamma_apso * 1.2)

    # lambda1参数范围 - 稍微收缩上限，为lambda2留出空间
    lambda1_lower = max(0.01, best_lambda1_apso * 0.8)
    lambda1_upper = min(0.98, best_lambda1_apso * 1.2)  # 使用0.95而不是0.98

    # lambda2参数范围 - 动态调整确保有效
    lambda2_lower = max(0.01, best_lambda2_apso * 0.8)
    lambda2_upper = min(0.98 - lambda1_lower, best_lambda2_apso * 1.2)  # 使用lambda1的下界计算

    # 确保lambda2的下界小于上界
    if lambda2_lower >= lambda2_upper:
        lambda2_lower = max(0.01, lambda2_upper * 0.8)  # 如果仍然有问题，将下界设为上界的一半

    # coef0参数范围
    coef0_lower = max(0, best_coef0_apso * 0.8)
    coef0_upper = max(coef0_lower + 0.0001, min(5.0, best_coef0_apso * 1.2))  # 确保上界大于下界

    # epsilon参数范围
    epsilon_lower = max(0.01, best_epsilon_apso * 0.8)
    epsilon_upper = min(1.0, best_epsilon_apso * 1.2)

    # 打印调整后的参数范围
    print("\n贝叶斯优化参数范围:")
    print(f"C: [{C_lower:.4f}, {C_upper:.4f}]")
    print(f"gamma: [{gamma_lower:.4f}, {gamma_upper:.4f}]")
    print(f"lambda1: [{lambda1_lower:.4f}, {lambda1_upper:.4f}]")
    print(f"lambda2: [{lambda2_lower:.4f}, {lambda2_upper:.4f}]")
    print(f"coef0: [{coef0_lower:.4f}, {coef0_upper:.4f}]")
    print(f"epsilon: [{epsilon_lower:.4f}, {epsilon_upper:.4f}]")

    # 步骤2: 贝叶斯优化在APSO找到的区域附近进行精细调优
    space = [
        Real(C_lower, C_upper, name='C', prior='log-uniform'),
        Real(gamma_lower, gamma_upper, name='gamma', prior='log-uniform'),
        Real(lambda1_lower, lambda1_upper, name='lambda1', prior='uniform'),
        Real(lambda2_lower, lambda2_upper, name='lambda2', prior='uniform'),
        Real(coef0_lower, coef0_upper, name='coef0', prior='uniform'),
        Real(epsilon_lower, epsilon_upper, name='epsilon', prior='log-uniform')
    ]
    # 固定degree为APSO找到的最佳值
    ...
    def objective(C, gamma, lambda1, lambda2, coef0, epsilon):
        try:
            # 确保lambda1 + lambda2 <= 0.98
            ...
            def custom_kernel(X, Y):
                return hybrid_kernel(X, Y, gamma=gamma, lambda1=lambda1,
                                     lambda2=lambda2, degree=best_degree, coef0=coef0)
    ...
    # 运行贝叶斯优化
    result = gp_minimize(objective, space, n_calls=50, random_state=44, verbose=True)

    # 获取最佳参数
    best_C = result.x[0]
    best_gamma = result.x[1]
    best_lambda1 = result.x[2]
    best_lambda2 = result.x[3]
    best_coef0 = result.x[4]
    best_epsilon = result.x[5]
    ...
    best_lambda3 = 1 - best_lambda1 - best_lambda2  # POLY的权重

    print(f"\n贝叶斯优化最佳参数:")
    print(f"C: {best_C:.4f}")
    print(f"gamma: {best_gamma:.4f}")
    print(f"lambda1 (RBF权重): {best_lambda1:.4f}")
    print(f"lambda2 (LINEAR权重): {best_lambda2:.4f}")
    print(f"lambda3 (POLY权重): {best_lambda3:.4f}")
    ...
    def final_kernel(X, Y):
        return hybrid_kernel(X, Y, gamma=best_gamma, lambda1=best_lambda1,
                             lambda2=best_lambda2, degree=best_degree, coef0=best_coef0)
